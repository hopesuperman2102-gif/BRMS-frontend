...
const AppBarComponent: React.FC<AppBarComponentProps> = ({ logo, organizationName }) => {
  ...
  const { getAccessToken, setAccessToken, setIsAuthenticated } = useAuth();
  ...
  const handleLogout = async () => {
    try {
      ...
      await logoutApi(getAccessToken());
      setAccessToken(null);
  ...
  return (
    ...
    >
      <Toolbar>
        <Box display="flex" alignItems="center" justifyContent="space-between" width="100%">
          ...
          <Box display="flex" alignItems="center" gap={1}>
            {!isRuleAuthor && !isReviewer && !isViewer && (
              <HeaderIcon
                icon={<SettingsIcon />}
                tooltip="Settings"
      ...
      {/* Settings Menu — hidden for RULE_AUTHOR, REVIEWER, VIEWER */}
      {!isRuleAuthor && !isReviewer && !isViewer && (
        <Menu
